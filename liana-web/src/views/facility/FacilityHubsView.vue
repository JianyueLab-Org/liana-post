...
const columns = [{ key: 'facilityCode', label: '中心编码' }, { key: 'name', label: '名称' }, { key: 'typeCode', label: '类型' }, { key: 'parentFacilityCode', label: '上级' }, { key: 'status', label: '状态' }];

async function load() {
  const allFacilities = await facilityApi.listFacilities(session.token);
  rows.value = allFacilities.filter((item) => item.typeCode === 'TRANSFER_CENTER');
}
